...
export class SecretObfuscator {
	...
	#collectRegexMatches(
		...
		for (;;) {
			...
			// raw bytes surround it, they still need redaction: obfuscate-mode regexes
			// can replace only those non-placeholder chunks with new placeholders,
			// while replace-mode keeps the stricter independent-match guard to avoid
			// growing user replacement markers on re-entry.
			...
			if (overlapsInputPlaceholder) {
				const outside = textOutsidePlaceholderRanges(text, start, end, knownPlaceholderRanges);
				if (outside.length === 0) continue;
				if (mode === "replace") {
					const resumeIndex = regex.lastIndex;
					regex.lastIndex = 0;
					const outsideIndependentlyMatches = regex.test(outside);
					regex.lastIndex = resumeIndex;
					if (!outsideIndependentlyMatches) continue;
				}
			}
			if (mode === "replace") {
